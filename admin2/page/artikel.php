...
if (!empty($_GET['id_setujui'])) {
    mysqli_query($conn, "UPDATE artikel SET status='1' WHERE id_artikel='$_GET[id_setujui]'");
    echo $refresh(0, "?page=artikel");
}
?>
        ...
        <div class="table-responsive p-2">
            <table class="table py-3" id="dataTable" style="min-width: 1000px;">
                ...
                <tbody>
                    <?php
                    $no = 1;
                    $q = mysqli_query($conn, "SELECT * FROM artikel a LEFT JOIN user b ON a.id_user=b.id_user ORDER BY id_artikel DESC");
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <a href="?page=artikel&&id_setujui=<?php echo $data['id_artikel'] ?>" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#setujui<?php echo $data['id_artikel'] ?>">Setujui</a>
                                <div class=" modal fade" id="setujui<?php echo $data['id_artikel'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Apakah Anda yakin Ingin Menyetujui Artikel <br> <b>"<?php echo $data['judul'] ?>"</b>

                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                <a href=" ?page=artikel&&id_setujui=<?php echo $data['id_artikel'] ?>" class="btn btn-success">Setujui</a>
