...
if (isset($_POST['simpan'])) {
    ...
    $ceknis = mysqli_num_rows(
        mysqli_query($conn, "SELECT * FROM data_siswa WHERE nis='$nis' or nisn='$nisn'")
    );

    if (empty($cek) and empty($ceknis)) {

        $simpan = $tambahUser($nama, $username, $password, 'siswa');
        if ($simpan) {
            $q = mysqli_query($conn, "SELECT * FROM user WHERE username='$username' and password='$password'");
            $data = mysqli_fetch_array($q);

            $id = $data['id_user'];
            $simpan = mysqli_query($conn, "INSERT INTO data_siswa VALUES('$nis','$nis','$id','$id_jurusan','$angkatan','','')");
            if ($simpan) {
                $alert = $alertSuccess('Berhasil Menambah Siswa');
                echo $refresh('1', '?page=tambah_siswa');
            }
        }
    } else {
...
<div class="card border-0 shadow-sm mt-4">
    <div class="card-body">
        ...
        <form action="" method="post">
            <input type="number" name="nis" class="form-control mb-3 p-2" required placeholder="Masukan NIS">
            <input type="number" name="nisn" class="form-control mb-3 p-2" required placeholder="Masukan NISN">
            ...
            <input type="number" name="angkatan" class="form-control mb-3 p-2" required placeholder="Masukan Angkatan">
