...
function artikelSearch($search)
{
    include "koneksi.php";
    $data = [];
    $q = mysqli_query($conn, "SELECT * FROM artikel a LEFT JOIN user b ON a.id_user=b.id_user WHERE status='1' and  judul LIKE '%$search%' and  keterangan LIKE '%$search%' ORDER BY id_artikel DESC");
    while ($d = mysqli_fetch_array($q)) {
        $data[] = $d;
    }
